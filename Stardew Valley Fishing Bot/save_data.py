...
upperBound_fish = np.array([120, 265, 197])
...
def fish(img, green_bar_win): #Fish: this looks pretty shitty. I really want to optimize it. What to change: only if it didn't match the fish at the first try it should try again. At the moment it is always trying 2 times and I think this is stupid and is slowing everything down.
    ...
    searching_nemo = img[y-5:470+y, 347+x:488+x]  # reduced area to look for fish, so less resources are used

    img_HSV = cv2.cvtColor(searching_nemo, cv2.COLOR_BGR2HSV)
    ...
    for cnt in conts:
        ...
        if area > 25:
            ...
            fish_center_height = (int(x), int(y))
            radius = int(radius)
            fish_center_point = cv2.circle(green_bar_win, fish_center_height, radius, (100, 0, 255), 2)
...
def process_img(img_rgb):
    ...
    return img_rgb, green_bar_win, rect_center_heigth
...
def main():
    ...
    fish_template1 = cv2.imread('Images\\fish.png')
    fish_template_gray1 = cv2.cvtColor(fish_template1, cv2.COLOR_BGR2GRAY)
    h1 = fish_template_gray1.shape[::-1][1]

    fish_template2 = cv2.imread('Images\\fish4.png')
    fish_template_gray2 = cv2.cvtColor(fish_template2, cv2.COLOR_BGR2GRAY)
    h2 = fish_template_gray2.shape[::-1][1] #w2 not used
    ...
    while(True):
        ...
        contour, green_bar_window, green_bar_height = process_img(screen) # process every frame (would be nice if it could process every 5 or so frames, so the process becomes faster).
        
        fish_detected, fish_height, searching_nemo = fish(contour, green_bar_window)
        '''
        if exc_detected_recently == False:

            exc_detected = exc_point(contour, exc_template_gray, we, he) # Contour is the img_rgb with the area of interest drawn over
            print("Detecting exclamation")

        if (exc_detected == True) or (exc_detected_recently == True):

            print("exc detected recently or exc detected now")
            print(exc_detected, exc_detected_recently)

            exc_detected_recently = True

            if (exc_detected_recently == True) and (exc_detected == False):

                print("Detecting fish")
                fish_detected, fish_height = fish(contour, green_bar_window, fish_template_gray1, h1, fish_template_gray1, h2) #also acts as a little delay

                if fish_detected: # got fish
                    print("Fish")
                    exc_detected_recently = True
                ...
                if (fish_detected == False):  #got trash
                    print("Trash")
        ...
        cv2.imshow('Fish ', searching_nemo)
        #This is for testing:
