...
def fishing_region(img_rgb, region_template_gray, w, h):  # the image format is actually BGR because of Opencv, but I didn't bother changing all the names
    ...
    img_gray = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2GRAY)   # both images needs to be gray to do template matching    
    ...
    for pt in zip(*loc[::-1]):
        ...
        x1, y1 = pt
        region_rect = cv2.rectangle(img_rgb, (x1, y1), (pt[0] + w, pt[1] + h), (0, 0,255), 4)
        region_detected = True
        print('F')
        break

    return region_detected
...
def fish(img, green_bar_win): #Fish: this looks pretty shitty. I really want to optimize it. What to change: only if it didn't match the fish at the first try it should try again. At the moment it is always trying 2 times and I think this is stupid and is slowing everything down.
    ...
    searching_nemo = img[y-5:470+y, 407+x:438+x]  # reduced area to look for fish, so less resources are used
...
def main():
    ...
    region_template = cv2.imread('Images\\fishing region.png')
    ...
    while(True):
        ...
        fishing_started = fishing_region(contour, region_template_gray, wr, hr)
